...
describe("Simple flow", () => {
  ...
  before(async () => {
    ...
    oceanAddress = await datatoken.create(
      blob,
      tom,
      "1000000000000000",
      "Ocean Token",
    ...
    ocean = new Ocean(
      web3,
      network,
      BalancerContracts.factoryAddress,
      oceanAddress
    );
  ...
  it("Alice & Bob creates a datatoken", async () => {
    ...
    dt1Address = await datatoken.create(
      blob,
      alice,
      "10000000000",
      "DataToken1",
    ...
    dt2Address = await datatoken.create(
      blob,
      bob,
      "10000000000",
      "DataToken2",
  ...
  it("Alice & Bob mints data tokens", async () => {
    ...
    assert(
      ...
      `ERROR, Expected ALICE balance of Tom is ${tokenAmount} & actual balance is ${balance1} `
    );
    assert(
      balance2 == tokenAmount,
      `ERROR, Expected BOB balance of Tom is ${tokenAmount} & actual balance is ${balance2} `
    );
  });
  ...
  it("Tom creates a datapool 1 & datapool2", async () => {
    ...
    //setup pool
    const estGas = await BalancerContracts.pool1.methods
      .setup(
        dt1Address,
    ...
    const setupTx = await BalancerContracts.pool1.methods
      .setup(
        dt1Address,
        web3.utils.toWei(String(dtAmount)),
        web3.utils.toWei(String(dtWeight)),
    ...
    const isFinalized = await BalancerContracts.pool1.methods
      .isFinalized()
      .call();
    ...
    await ocean.approve(
      dt2Address,
      BalancerContracts.pool2Address,
      dtAmount,
      tom
    );
    await ocean.approve(
    ...
    const setupPool2Tx = await BalancerContracts.pool2.methods
      .setup(
        dt2Address,
        web3.utils.toWei(String(dtAmount)),
        web3.utils.toWei(String(dtWeight)),
    ...
    const isPool2Finalized = await BalancerContracts.pool2.methods
      .isFinalized()
      .call();
  ...
  it("Bob swaps Ocean for ALICE Dt from Pool1", async () => {
    const oceanSpent = "30";
    const dtReceived = await ocean.getDtReceived(
      BalancerContracts.pool1Address,
      oceanSpent
    );
    ...
    let approveTx = await ocean.approve(
      oceanAddress,
      BalancerContracts.pool1Address,
      oceanSpent,
      bob
    ...
    let swapTx = await ocean.swapExactOceanToDt(
      bob,
      BalancerContracts.pool1Address,
      "1",
      oceanSpent,
    ...
    assert(
      actualReceivedDtBalance >= dtReceived,
      `ERROR : Received ${actualReceivedDtBalance} dt in swap but expected minimum ${dtReceived}`
    );
  });

  it("Alice swaps ALICE dt for Ocean from Pool1", async () => {
    const dtSpent = "10";
    const oceanReceived = await ocean.getOceanReceived(
      BalancerContracts.pool1Address,
      dtSpent
    );
    ...
    let swapTx = await ocean.swapExactDtToOcean(
      alice,
      BalancerContracts.pool1Address,
      "1",
      dtSpent,
    ...
    assert(
      oceanBalanceAfterSwap >= oceanBalanceBeforeSwap + oceanReceived,
      `ERROR : Received ${oceanBalanceAfterSwap} dt in swap but expected minimum ${oceanReceived}`
    );
  });
  ...
  it("Bob swaps BOB for exact amount of ALICE", async () => {
    ...
    let inputDtNeeded = await ocean.getDtNeededForExactDt(
      outputDtNeeded,
      BalancerContracts.pool2Address,
      BalancerContracts.pool1Address
    );
    console.log("Max input Dt - ", inputDtNeeded);
    let dt2 = await ocean.swapDtToExactDt(
      bob,
      dt2Address,
      dt1Address,
      outputDtNeeded,
      ...
      BalancerContracts.pool2Address,
      BalancerContracts.pool1Address,
      BalancerContracts.proxyAddress
    );
    //console.log(dt2)
  ...
  it("Bob swaps exact ALICE for minimum amount of BOB", async () => {
    ...
    let inputDtAmount = "5";
    let minOutputDtReceived = await ocean.getDtReceivedForExactDt(
      inputDtAmount,
      BalancerContracts.pool1Address,
      BalancerContracts.pool2Address
    );
    console.log("Min BOB received - ", minOutputDtReceived);
    let dt2 = await ocean.swapExactDtToDt(
      bob,
      dt1Address,
      dt2Address,
      minOutputDtReceived,
