...
import { ITokenDetails } from "src/@types";
import { Config } from "../";
...
export async function approve(
  ...
  if (!force) {
    ...
    if (
      new Decimal(currentAllowence).greaterThanOrEqualTo(new Decimal(amount))
    ) {
      return currentAllowence;
    }
  }
  let result = null;
  const amountFormatted = await amountToUnits(
    web3,
    tokenAddress,
    amount,
    tokenDecimals
  ...
  try {
    result = await tokenContract.methods
      .approve(spender, amountFormatted)
      .send({
        from: account,
        gas: estGas + 1,
